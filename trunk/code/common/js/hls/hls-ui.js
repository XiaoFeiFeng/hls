...
define(['moment', 'locale', 'datetimepicker'], function () {
    var hlsUiModule = angular.module('hls.ui', [
        'hls.core', 'hls.ui.area', 'hls.ui.datetimepicker', 'hls.ui.pagination', 'hls.ui.formvalid', 'hls.ui.dialogs', 'hls.ui.include', 'hls.ui.grid']);
                        ...
                        element.bind('focus', function (evt) {
                            ...
                            scope.$apply(function () {
                                ...
                                promise.then(function (result) {
                                    ...
                                    ctrl.$setValidity('keyvalidinfo', result);
                                    if (!result) {
                                        var properName = angular.lowercase('dirkeyvalid' + messageName);
                                        var message = attrs[properName];
                                        message = validateService.isEmpty(message) ? '该值已经存在' : message;
    ...
    angular.module('hls.ui.dialogs.controllers', ['ui.bootstrap.modal'])
        .run(['$templateCache', function ($templateCache) {
            $templateCache.put('/dialogs/error.html', '<div class="error_modal"><div class="modal-header dialog-header-error">    <h4><i class="glyphicon glyphicon-warning-sign" /> 错误</h4></div><div class="modal-body">{{msg}}</div> <div class="modal-footer"><button class="btn btn-primary" ng-click="close()"  focus-me="{{true}}">关闭</button></div></div><iframe frameborder="0" scrolling="no" style="background-color: transparent; position: absolute; z-index: -1; width: 100%; height: 100%; top: 0; left: 0;"></iframe>');
            $templateCache.put('/dialogs/notify.html', '<div class="notify_modal"><div class="modal-header dialog-header-notify">    <h4><i class="glyphicon glyphicon-info-sign" /> {{header}}</h4></div><div class="modal-body">    {{msg}}</div> <div class="modal-footer"><button class="btn btn-primary" ng-click="close()"  focus-me="{{true}}">关闭</button></div></div><iframe frameborder="0" scrolling="no" style="background-color: transparent; position: absolute; z-index: -1; width: 100%; height: 100%; top: 0; left: 0;"></iframe>');
            $templateCache.put('/dialogs/confirm.html', '<div class="confirm_modal"><div class="modal-header dialog-header-confirm"><h4><i class="glyphicon glyphicon-check" /> {{header}}</h4></div><div class="modal-body"><p ng-bind-html="msg"></p></div>  <div class="modal-footer">      <button class="btn btn-warning" ng-click="no()" focus-me="{{true}}">取消</button>      <button class="btn btn-primary" ng-click="yes()">确定</button></div></div><iframe frameborder="0" scrolling="no" style="background-color: transparent; position: absolute; z-index: -1; width: 100%; height: 100%; top: 0; left: 0;"></iframe>');
        }])
        ...
        .factory('$dialogs', ['$modal', function ($modal) {
            return {
                ...
                openModal: function (templateUrl, controller, modalData, size, okcallbackFun, cancelcallbackFun) {
                    var modalInstance = $modal.open({
                        templateUrl: templateUrl,//'/Security/Role/modal',
    ...
    angular.module('hls.ui.grid', [])
        .run(["$templateCache", function ($templateCache) {
            $templateCache.put("template/hls/grid.html",
                ...
                "<table ng-if=\"showGrid\" class=\"table table-bordered table-striped table-hover\" style='margin-bottom:0px;'>\n" +
                "<thead>\n" +
                "<tr>\n" +
                ...
                "<span>{{getFieldValue(record,viewCol)}}</span>" +//{{record[viewCol.FieldName]}}
                "</td>" +
                ...
                "</a>" +
                "<a style=\"cursor:pointer\" ng-if=\"showView(record)\" ng-click=\"viewFunction(record,$event)\" class=\"btn-link btn-lg btn-lg-iconpadding\" title=\"查看\" >" +
                "<span  class=\"glyphicon glyphicon-eye-open text-primary\" ></span>" +
                "</a>" +
                "<a style=\"cursor:pointer\"  ng-if=\"showEdit(record)\" ng-click=\"editFunction(record,$event)\" class=\"btn-link btn-lg btn-lg-iconpadding\" title=\"修改\" >" +
                "<span class=\"glyphicon glyphicon-edit text-primary\" ></span>" +
                "</a>" +
                "<a style=\"cursor:pointer\"  ng-if=\"showDelete(record)\" ng-click=\"deleteFunction(record,$event)\" class=\"btn-link btn-lg btn-lg-iconpadding\" title=\"删除\" >" +
        ...
        .controller('hlsGridController', ['$scope', function ($scope) {
            var self = this;
            ...
            $scope.getFieldValue = function (record, colInfo) {
                ...
                if (angular.isDefined(colInfo.Formatter) && angular.isFunction(colInfo.Formatter)) {
                    return colInfo.Formatter(fieldValue);
                }
                return fieldValue;
            }
