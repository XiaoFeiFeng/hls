    ...
    angular.module('channelIndexModule', [])
        .controller("channelIndexCtrl", ['$scope', '$ui', '$request', '$validate', function ($scope, $ui, $request, $validate) {
            ...
            $scope.getChannels = function (type) {
                $request.get(
                    'shipping/data/?action=logistics_category_channels&category=' + type.code,
                    function (response) {
            ...
            $scope.modifyState = function (channel) {
                ...
                $request.post('shipping/data/?action=edit_logistics_category_channels&_id=' + channel._id.$id,
                    data,
                    function (response) {
                        $scope.getChannels($scope.currentCategory);
                    }, function (err) {
            ...
            $scope.delChannel = function (id) {
                $ui.confirm('确认删除?', '确认',
                    function () {
                        $request.get('shipping/data/?action=remove_logistics_category_channels&_id=' + id,
                            function (response) {
                                $ui.notify('删除成功', '提示');
                                $scope.getChannels($scope.currentCategory);
                            }, function (err) {
