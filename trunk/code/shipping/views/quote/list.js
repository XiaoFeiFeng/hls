...
define([], function () {
    ...
    quoteListModule.controller('quoteListCtrl', ['$scope', '$ui', '$validate', '$request', 'validateService', function ($scope, $ui, $validate, $request, validateService) {
        ...
        $scope.getFee = function (type) {
            ...
            var url = 'shipping/data/?action=getFee&country=' + $scope.countryData.selectedData.Code + '&weight=' + $scope.weight;
            ...
            $request.post(url, data, function (data) {
                $scope.feeResult = data.transportFee;
                if (data.success) {
        ...
        $scope.dataSort = function (type, isDesc) {
            $scope.priceDesc = !isDesc;
            $scope.feeResult.sortby(type, isDesc);
        }
        ...
        $scope.transTypeChange = function (item) {
            ...
            $request.get(
                'shipping/data/?action=logistics_category_channels&status=1&category=' + item.code,
                function (response) {
                    $scope.transWays = response;
                }, function (err) {
                    $ui.error('错误', err);
                }
            );
